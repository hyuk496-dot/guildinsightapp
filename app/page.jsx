  ...
  return(
    <div style={{background:t.navBg,borderBottom:`1px solid ${t.navBorder}`,padding:"11px 22px",display:"flex",alignItems:"center",justifyContent:"space-between",flexShrink:0,transition:TR}}>
      ...
      <div style={{display:"flex",alignItems:"center",gap:8}}>
        ...
        <button style={{fontSize:10,padding:"5px 11px",border:`1px solid ${t.borderStrong}`,borderRadius:7,background:t.accentFaint,color:t.accent,fontFamily:"'Courier New',monospace",cursor:"pointer",fontWeight:500}}>GPT 리포트</button>
        {/* 구분선 */}
      ...
      <div style={{width:320,flexShrink:0,borderRight:`1px solid ${t.border}`,display:"flex",flexDirection:"column",overflow:"hidden"}}>
        <div style={{padding:"14px 16px 10px",borderBottom:`1px solid ${t.border}`}}>
          ...
          <select value={guildId} onChange={e=>{setGuildId(+e.target.value);setSelected(null);}}
            style={{...iStyle(t),marginBottom:8}}>
            {guilds.map(g=><option key={g.id} value={g.id}>{g.name} ({g.game})</option>)}
          </select>
    ...
    <div style={{display:"flex",flex:1,overflow:"hidden"}}>
      <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden"}}>
        ...
        <div style={{padding:"12px 18px",borderBottom:`1px solid ${t.border}`,display:"flex",alignItems:"center",gap:10,flexWrap:"wrap",background:t.navBg}}>
          <select value={guildId} onChange={e=>{setGuildId(+e.target.value);setSelected(null);}} style={{...iStyle(t),width:"auto"}}>
    ...
    <div style={{flex:1,overflowY:"auto",padding:"18px 22px"}}>
      ...
      <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:14,padding:"10px 14px",background:t.bgCard,border:`1px solid ${t.border}`,borderRadius:9}}>
        ...
        <select value={guildId} onChange={e=>{setGuildId(+e.target.value);setSimScores({});setGuildSim("");}}
          style={{...iStyle(t),flex:1,maxWidth:200}}>
          {guilds.map(g=><option key={g.id} value={g.id}>{g.name} ({g.game})</option>)}
        </select>
        <div style={{display:"flex",alignItems:"center",gap:6}}>
        ...
        <div style={{background:t.bgCard,border:`1px solid ${t.border}`,borderRadius:11,padding:"12px 16px",display:"flex",alignItems:"center",gap:10,flexWrap:"wrap"}}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:6}}>
            ...
            <select value={saveGuildId} onChange={e=>setSaveGuildId(+e.target.value)}
              style={{padding:"5px 9px",borderRadius:6,border:`1px solid ${t.border}`,background:t.inputBg,color:t.text,fontSize:11,fontFamily:"'Courier New',monospace",outline:"none"}}>
              {guilds.map(g=><option key={g.id} value={g.id}>{g.name}</option>)}
            </select>
          </div>
          {/* 저장 경로 선택 — 컨텐츠 ← 핵심 추가 */}
          <div style={{display:"flex",alignItems:"center",gap:6}}>
            ...
            <select value={saveContent} onChange={e=>setSaveContent(e.target.value)}
              style={{padding:"5px 9px",borderRadius:6,border:`1px solid ${t.borderStrong}`,background:t.accentFaint,color:t.accent,fontSize:11,fontFamily:"'Courier New',monospace",outline:"none",fontWeight:500}}>
        ...
        <div style={{padding:"14px 14px 10px",borderBottom:`1px solid ${t.border}`}}>
          ...
          <div style={{marginBottom:10}}>
            ...
            <select value={rptContent} onChange={e=>setRptContent(e.target.value)}
              style={{...iStyle(t),fontSize:11}}>
              {contentOptions.map(c=><option key={c} value={c}>{c==="전체"?"전체 통합":"▶ "+c}</option>)}
            </select>
            {rptContent!=="전체"&&(
        ...
        {selected&&(
          <>
            <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",marginBottom:18}}>
              ...
              <div style={{display:"flex",gap:8}}>
                <button style={{fontSize:11,padding:"6px 13px",border:`1px solid ${t.border}`,borderRadius:7,background:"transparent",color:t.textSub,cursor:"pointer",fontFamily:"'Courier New',monospace"}}>PDF 출력</button>
                <button style={{fontSize:11,padding:"6px 13px",border:`1px solid ${t.border}`,borderRadius:7,background:"transparent",color:t.textSub,cursor:"pointer",fontFamily:"'Courier New',monospace"}}>공유</button>
              </div>
            </div>
            ...
            {selected.sections.map((sec,i)=>(
              <div key={i} style={{background:t.bgCard,border:`1px solid ${t.border}`,borderRadius:11,padding:"14px 18px",marginBottom:10}}>
...
// ROOT APP
// ══════════════════════════════════════════════════════════════════════════════
export default function GuildInsightApp(){
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",height:"100vh",background:t.bg,color:t.text,fontFamily:"'Courier New',monospace",transition:TR,overflow:"hidden"}}>
      ...
      <Modal open={logoutModal} onClose={()=>setLogoutModal(false)} t={t} title="로그아웃">
        <div>
          <div style={{fontSize:12,color:t.textSub,lineHeight:1.7,marginBottom:6}}>
            <strong style={{color:t.accent}}>{activeGuild.name}</strong> 운영자 계정에서 로그아웃합니다.
          </div>
