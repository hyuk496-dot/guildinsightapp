...
function memberWeekScore(scores, guildId, content, activityWeek, memberId, dbNames) {
  let best = 0;
  for (const row of scores || []) {
    if (String(row.guild_id) !== String(guildId)) continue;
    if (!matchesContentName(row.content_name, content, dbNames)) continue;
...
function memberHistoryAvg(scores, guildId, content, memberId, excludeWeek, dbNames) {
  const vals = [];
  for (const row of scores || []) {
    if (String(row.guild_id) !== String(guildId)) continue;
    if (!matchesContentName(row.content_name, content, dbNames)) continue;
...
function guildFillAverage(scores, activeMembers, guildId, content, excludeWeek, dbNames) {
  const avgs = activeMembers
    .map((m) => memberHistoryAvg(scores, guildId, content, m.id, excludeWeek, dbNames))
    .filter((v) => v > 0);
  if (avgs.length) {
    return Math.round(avgs.reduce((a, b) => a + b, 0) / avgs.length);
  }
  const weekScores = activeMembers
    .map((m) => memberWeekScore(scores, guildId, content, excludeWeek, m.id, dbNames))
    .filter((v) => v > 0);
