...
/**
 * scores·members 기준 전체 길드원 목록에 contributions 행을 LEFT JOIN.
 * 저장된 행이 있으면 score·history·id(contributions PK)를 덮어씀.
 */
export function mergeGuildContribs({ members = [], scores = [], contribRows = [], guildId }) {
  const base = buildContribsFromScores(scores, guildId, members);
  const savedByMember = new Map(
    (contribRows || []).map((row) => [String(row.member_id), row])
  );

  const merged = base.map((entry) => {
    const saved = savedByMember.get(String(entry.member_id));
    if (!saved) return entry;
    return {
      ...entry,
      id: saved.id ?? entry.id,
      member_id: entry.member_id,
      nick: saved.nick ?? entry.nick,
      score: Number(saved.score ?? 0),
      history: Array.isArray(saved.history) ? saved.history : [],
    };
  });

  return applyContribPercents(merged).sort((a, b) => b.score - a.score);
}
