...
export function resolveRankingWeek(scores, contentName) {
  const current = weekMondayKey();
  const weeks = new Set();

  (scores || []).forEach((row) => {
    if (row.content_name !== contentName) return;
 ...
 */
export function buildServerRankings(scores, guilds, { contentName, gameName, currentGuildId }) {
  const weekMonday = resolveRankingWeek(scores, contentName);
