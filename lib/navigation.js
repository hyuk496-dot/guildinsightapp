...
export const ROUTES = {
  ...
  simulationLegacy: "/simulation/legacy",
  gptreport: "/gptreport",
...
export const NAV_ROUTE_MAP = {
  ...
  rankingGoals: ROUTES.simulation,
  simulation: ROUTES.simulationLegacy,
...
export const PAGE_TITLES = {
  ...
  rankingGoals: () => ["랭킹 목표", "우리 길드 순위 갭 · 참여 시나리오"],
  simulation_legacy: () => ["랭킹 시뮬레이션 (구)", "보류된 이전 버전 — URL 직접 접근"],
...
export function pathnameToPageKey(pathname) {
  ...
  if (pathname === ROUTES.simulationLegacy || pathname.startsWith(`${ROUTES.simulationLegacy}/`)) {
    return "simulation_legacy";
  }
  if (pathname === ROUTES.simulation) return "rankingGoals";
